...
public class DashboardController {
    ...
    private void onChallengeAI() {
        // Hiển thị dialog chọn board size
        javafx.scene.control.ChoiceDialog<String> boardDialog = new javafx.scene.control.ChoiceDialog<>(
                "19x19", "9x9", "13x13", "19x19");
        boardDialog.setTitle("Chọn kích thước bàn cờ");
        boardDialog.setHeaderText("Chơi với AI");
        boardDialog.setContentText("Chọn kích thước bàn cờ:");

        java.util.Optional<String> result = boardDialog.showAndWait();
        if (!result.isPresent()) {
            return; // User cancelled
        }

        // Parse board size từ selection
        int boardSize = 19;
        String selected = result.get();
        if (selected.startsWith("9")) {
            boardSize = 9;
